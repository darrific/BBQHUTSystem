var order = {};
var combosTemplate = '{{#ComboItems}}<div class="row" id="combo_item{{comboID}}"><div class="col-xs-12 col-xs-offset-0 bg_4"><div class="row padding_20"><div class="col-xs-1 text-center"><br><span class="badge_r padding_1">{{quantity}}</span></div><div id="order_image" class="col-xs-3"><img src="{{image}}" /></div><div class="col-xs-5"><b>{{name}}</b><br><i>{{details}}</i></div><div class="col-xs-2 w"><br>${{price}}</div></div></div></div>{{/ComboItems}}';
var sidesTemplate = '{{#SidesItems}}<div class="row" id="side_item{{sideID}}"><div class="col-xs-12 col-xs-offset-0 bg_4"><div class="row padding_20"><div class="col-xs-1 text-center"><br><span class="badge_r padding_1">{{quantity}}</span></div><div class="col-xs-5"><b>{{name}}</b><br>{{#details}}<i>{{details}}</i>{{/details}}</div><div class="col-xs-2 w"><br>${{price}}</div></div></div></div>{{/SidesItems}}';
...
$("div[id^='combo_item']").each(function(){
	...
	hammerTime.on("press", function(){
		...
		var quantity = parseInt(ObjectCache.first().html().replace('<br><span class="badge_r padding_1">', '').replace('</span>', ''), 10);
		...
		quantity++;
		ObjectCache.first().html('<br><span class="badge_r padding_1">'+ quantity + '</span>');
