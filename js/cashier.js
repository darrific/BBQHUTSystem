...
function render(){
	...
	$('#injectOverdueHere').html(Mustache.render(template, {OrderCount: overdueOrder}));
	$("h2[id^='totalPrice']").each(function(){
		...
		//alert($(this).parent().parent().parent().parent().children().first().children().first().children().first().children().first().children().first().html().replace("<h2>ID: ","").replace("</h2>",""));
...
function getOrder(id){
	...
	for(var o in overdueOrder){
		if(overdueOrder[o].id == id){
			return overdueOrder[o];
		}
	}
}
...
function updateCashierUI(){
	...
	$.post('/php/ajax.php', {action: 'checkOverdue'}, function(data, textStatus, xhr) {
		$.ajax({
			...
			method: "POST",
			url: 'php/ajax.php', 
			data: {action:'updateBackendUI', status:"Overdue"},
			success: function(data) {
				...
				if(!(JSON.stringify(overdueOrder) === JSON.stringify(dataObject))){
						var beep = new Audio("media/beep.wav");
						beep.play();
					overdueOrder = dataObject;
					render();
					registerButtons();
				}
				a = 1;
